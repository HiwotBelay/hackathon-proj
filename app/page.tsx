...
import { PetEnvironment } from "@/components/pet-environment"
...
import { Settings, Volume2, VolumeX, Camera, Share2, Award, Gamepad2, Shirt, Brain } from 'lucide-react'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (savedPetExperience) setPetExperience(parseInt(savedPetExperience))
    ...
    if (savedLastInteraction) setLastInteraction(parseInt(savedLastInteraction))
    ...
    if (savedLastInteraction) {
      const timeAway = Date.now() - parseInt(savedLastInteraction)
  ...
  }, [petName, petType, petColor, petLevel, petExperience, petStats, petAchievements, petAccessories, activeAccessory, petPersonality])
  ...
  const updatePetStats = (message: string, emotion: string) => {
    ...
    setPetStats(prev => {
      ...
      // Happiness changes
      if (lowerMessage.includes("good") || lowerMessage.includes("love") || lowerMessage.includes("happy") || 
          emotion === "happy" || emotion === "excited" || emotion === "loving") {
        newStats.happiness = Math.min(prev.happiness + 5, 100)
      } else if (lowerMessage.includes("bad") || lowerMessage.includes("sad") || lowerMessage.includes("angry") ||
                emotion === "sad" || emotion === "scared") {
        newStats.happiness = Math.max(prev.happiness - 3, 0)
      }
      
      // Energy changes
      if (lowerMessage.includes("play") || lowerMessage.includes("run") || lowerMessage.includes("exercise") ||
      ...
      // Hunger changes
      if (lowerMessage.includes("food") || lowerMessage.includes("eat") || lowerMessage.includes("treat") ||
          emotion === "hungry") {
      ...
      if (lowerMessage.includes("learn") || lowerMessage.includes("smart") || lowerMessage.includes("teach") ||
  ...
  const generatePetResponse = (
    ...
    const timeAwareGreeting = 
      ...
      timeOfDay === "night" ? "Hello there, night owl! " : "Hello! "
    ...
    if (lowerMessage.includes("hello") || lowerMessage.includes("hi") || lowerMessage.includes("hey")) {
    ...
    else if (lowerMessage.includes("accessory") || lowerMessage.includes("wear") || lowerMessage.includes("dress") || lowerMessage.includes("outfit")) {
      response.text = `I love getting dressed up! Check out my accessory collection and help me look fabulous!`
      response.emotion = "excited"
      setShowAccessories(true)
    }
    else if (lowerMessage.includes("achievement") || lowerMessage.includes("award") || lowerMessage.includes("trophy")) {
      response.text = `I've earned ${petAchievements.length} achievements so far! Want to see them?`
      response.emotion = "proud"
      setShowAchievements(true)
    }
    else if (lowerMessage.includes("share") || lowerMessage.includes("photo") || lowerMessage.includes("picture") || lowerMessage.includes("snapshot")) {
    ...
    else if (lowerMessage.includes("curious") || lowerMessage.includes("wonder") || lowerMessage.includes("what if")) {
      if (isCurious) {
      ...
      if (isIndependent) {
        defaultResponses.push(
          { text: `That's an interesting perspective. I've been thinking about that differently.`, emotion: "thinking" },
          { text: `I appreciate you sharing that with me. I've been exploring some ideas on my own too.`, emotion: "curious" }
        )
      }
    ...
    <main className="flex min-h-screen flex-col bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <header className="sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-slate-900 dark:text-white">{petName}</h1>
      ...
      <div className="flex-1 container mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 order-2 md:order-1">
          <Card className="overflow-hidden h-[500px] md:h-[600px] shadow-lg border-slate-200 dark:border-slate-700">
            <CardContent className="p-0 h-full relative">
              ...
              {/* Pet stats indicator */}
              <div className="absolute bottom-4 left-4 right-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-3 rounded-lg shadow-md">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1 flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-slate-600 dark:text-slate-300">Happiness</span>
                      <span className="text-xs font-medium text-slate-800 dark:text-slate-200">{petStats.happiness}%</span>
                    </div>
                  ...
                  <div className="flex flex-col gap-1 flex-1 mx-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-slate-600 dark:text-slate-300">Energy</span>
                      <span className="text-xs font-medium text-slate-800 dark:text-slate-200">{petStats.energy}%</span>
                    </div>
                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5">
                      <div
                  ...
                  <div className="flex flex-col gap-1 flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-slate-600 dark:text-slate-300">Hunger</span>
                      <span className="text-xs font-medium text-slate-800 dark:text-slate-200">{petStats.hunger}%</span>
                    </div>
                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5">
                      <div
              ...
              {/* Level progress */}
              <div className="absolute top-4 left-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-md">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-slate-800 dark:text-slate-200">Level {petLevel}</span>
                  ...
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-400">{petExperience}/{petLevel * 100} XP</span>
                </div>
              </div>
              ...
              {isCapturing && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                  <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-lg text-center">
                    <h3 className="text-lg font-medium mb-4">Take a photo with {petName}!</h3>
        ...
        <div className="md:col-span-1 order-1 md:order-2">
          <Card className="h-full flex flex-col shadow-lg border-slate-200 dark:border-slate-700">
